...
menu.display = function ()	
	...
	local money = ConvertMoneyString( menu.ship.trader[9][4], false, true, nil, true) .. " Cr" 
			...
			for i = 3, 10, 1 do
				...
				if msg then
					if msg[3] == "TRADE_SUCCESS" then
						local cells = {}
						local message = "Waypoint: " .. tostring(msg[17]) .. " - " .. LibMT.WPType[msg[6]] .. " - " .. GetWareData( msg[8],"name") .. " - " .. msg[9] .. " - " .. GetComponentData( msg[7], "name")
						table.insert(cells, LibMT:Cell( ConvertTimeString(GetCurTime() - msg[1]), nil, 1) )
						table.insert(cells, LibMT:Cell( message, nil, 4) )
						table.insert(row_collection, LibMT:Row(cells, {}, Helper.defaultHeaderBackgroundColor, false, 0))
					else
						local cells = {}
						local message = "Waypoint: " .. tostring(msg[17]) .. ": " .. tostring(LibMT.Messages[ msg[3] ])
						table.insert(cells, LibMT:Cell( ConvertTimeString(GetCurTime() - msg[1]), nil, 1) )
						table.insert(cells, LibMT:Cell( message, nil, 4) )
						table.insert(row_collection, LibMT:Row(cells, {}, Helper.defaultHeaderBackgroundColor, false, 0))
					end
