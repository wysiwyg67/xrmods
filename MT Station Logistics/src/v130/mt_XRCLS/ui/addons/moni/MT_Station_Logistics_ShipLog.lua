...
menu.onShowMenu = function ()
	...
	menu.ship.waypoints = menu.ship.trader[4]
...
--		table.insert(cells, LibMT:Cell( ReadText(150402,413), nil, 2) ) -- "Tracking Log"
		...
		if LibMT.DEBUG then
			if 0 < menu.ship.debug_level then
				table.insert(cells, LibMT:Cell( ReadText(150402,463) .. "  ( " .. ReadText(150402,411) .. " - " .. menu.ship.dbgType[2] .. " )", nil, 3) ) -- "Debug Level for this ship"
			else
				table.insert(cells, LibMT:Cell( ReadText(150402,463) .. "  ( " .. ReadText(150402,411) .. " - " .. menu.ship.dbgType[1] .. " )", nil, 3) ) -- "Debug Level for this ship"
			end
		else
			table.insert(cells, LibMT:Cell( ReadText(150402,463), nil, 3) ) -- "Message"
		end
		table.insert(cells, LibMT:Cell( menu.ship.tracktype[menu.ship.tracking + 1] .. "  " .. ConvertTimeString(GetCurTime()), nil, 1) ) -- "Tracking for this ship"
		table.insert(row_collection, LibMT:Row(cells, {}, Helper.defaultHeaderBackgroundColor, true, 0))
...
menu.onUpdate = function ()
	-- Run timer
	if menu.ship.tracking > 0 then
		Helper.updateCellText(menu.selecttable, 2, 5, "Tracking:  " .. ConvertTimeString(GetCurTime(), "%M:%S"), LibMT.colours.white)
	end
	if menu.ship.captain then
		...
		if menu.ship.captainlog and menu.ship.tracking == 1 then
			if 0 < #menu.ship.TrackLog then
				...
				-- Get next waypoint details and format to message
				local nextwaypoint = menu.ship.waypoints[wpnum + 1]
				local nextwptype = LibMT.WPType[nextwaypoint[4]]
				local nextwpware = GetWareData( nextwaypoint[5],"name")
				local nextstation = GetComponentData(nextwaypoint[3], "name")
				local Cluster = ffi.string(C.GetMapShortName(   C.GetContextByClass( ConvertIDTo64Bit(nextwaypoint[3]), "cluster", true )  ))
				local Sector = ffi.string(C.GetMapShortName(   C.GetContextByClass( ConvertIDTo64Bit(nextwaypoint[3]), "sector", true )  ))
				local Zone = ffi.string(C.GetMapShortName(   C.GetContextByClass( ConvertIDTo64Bit(nextwaypoint[3]), "zone", true )  ))
				local location = " - " .. Cluster .. "." .. Sector .. "." .. Zone
					Helper.updateCellText(menu.selecttable, 1, 1, "WP " .. wpnum + 1, LibMT.colours.white)
				Helper.updateCellText(menu.selecttable, 1, 2,   nextwptype .. " " .. nextwpware .. " at " .. nextstation .. location, LibMT.colours.white)
				for i=3, 33 ,1 do
					local msg = menu.ship.TrackLog[i - 2]
					if msg then
						if msg[3] == "TRADE_SUCCESS" then
				...
				-- no tracklog yet so diplay 1st wp
				local nextwaypoint = menu.ship.waypoints[1]
				local nextwptype = LibMT.WPType[nextwaypoint[4]]
				local nextwpware = GetWareData( nextwaypoint[5],"name")
				local nextstation = GetComponentData(nextwaypoint[3], "name")
				local Cluster = ffi.string(C.GetMapShortName(   C.GetContextByClass( ConvertIDTo64Bit(nextwaypoint[3]), "cluster", true )  ))
				...
				local Zone = ffi.string(C.GetMapShortName(   C.GetContextByClass( ConvertIDTo64Bit(nextwaypoint[3]), "zone", true )  ))
				local location = " - " .. Cluster .. "." .. Sector .. "." .. Zone
				Helper.updateCellText(menu.selecttable, 1, 1, "WP " .. 1, LibMT.colours.white)
				Helper.updateCellText(menu.selecttable, 1, 2,   nextwptype .. " " .. nextwpware .. " at " .. nextstation .. location, LibMT.colours.white)
			end
		end
	end
