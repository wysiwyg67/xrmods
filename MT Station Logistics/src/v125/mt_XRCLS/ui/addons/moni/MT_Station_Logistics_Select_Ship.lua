...
menu.onShowMenu = function ()
	...
	menu.ship = {}
	menu.toprow = menu.param[1] or 0
	menu.selrow = menu.param[2] or 0 
	...
	menu.ship.YAT_ships = menu.param[8] or {}
	menu.ship.jlp_ships = {}
	menu.ship.euat_ships = {}
	
	-- Global Options
	menu.ship.GlobalOptions = menu.param[9] or {0}
...
--	DebugError( "Found " .. tostring(#menu.ship.jlp_ships) .. "  JohnLukePicard Unitraders" )
...
--		if captain and GetNPCBlackboard(captain, "$Autotrade_mtxrcls") then
			table.insert(menu.ship.euat_ships, menu.ship.ships[i])
			table.remove(menu.ship.ships, i)
		elseif captain and GetNPCBlackboard(captain, "$Miner") then
			table.insert(menu.ship.euat_ships, menu.ship.ships[i])
			table.remove(menu.ship.ships, i)
		elseif captain and GetNPCBlackboard(captain, "$Sminer") then
			table.insert(menu.ship.euat_ships, menu.ship.ships[i])
			table.remove(menu.ship.ships, i)
		elseif captain and GetNPCBlackboard(captain, "$Strader") then
			table.insert(menu.ship.euat_ships, menu.ship.ships[i])
			table.remove(menu.ship.ships, i)
		end	
	end
--	DebugError( "Found " .. tostring(#menu.ship.euat_ships) .. "  Euclid AutoTraders" )
...
--	Need to remove ships that have been re-squadded or re-assigned to another commander
--	assigned3 = LibMT.deepcopy(assigned2)
--	assigned3 = LibMT.filter_ships( assigned3, {"commander"} ) -- remove  re-squadded ships and keep for later
	
	-- Now get the locked ships
--	lockedShips = LibMT.Set.Difference( assigned2, assigned3)
	
--	for i,v in ipairs(lockedShips) do
--		DebugError( "locked ships:  " .. GetComponentData(v,"name"))
--	end
--	for i,v in ipairs(assigned3) do
--		DebugError( "Assigned3:  " .. GetComponentData(v,"name"))
--	end
...
--				local activity = ReadText(150402,112)						-- "None"
	...
	if menu.toprow == nil then menu.toprow = menu.param[1] end
...
Buttons.ShipSetup = function()
	...
	Helper.closeMenuForSection(menu, false, "gMT_ShipMenu_ShipSetup", { 0, 0, {}, {toprow, selrow, expand_state_table, "shipsetup"}, {}, menu.ship.trader, {} })
	menu.cleanup()
	return
end
