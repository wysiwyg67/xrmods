...
menu.onShowMenu = function ()
	...
	-- row state to display
	menu.toprow 								= menu.param[1][1]
	menu.selrow 								= menu.param[1][2]
	
	-- calling menu's rowstate
	menu.wplist.calling_menu_rowstate			= menu.param[2]
	
	-- name of menu that called us
	menu.wplist.calling_menu 					= menu.param[3]
	
	-- Place to return if required
	menu.wplist.return_menu						= menu.param[4]
	
	-- Additional parameters passed to this menu
	menu.wplist.additional_params				= menu.param[5]
	menu.wplist.action							= menu.wplist.additional_params[1]
	
	-- Trade ship list (usually not passed)
	menu.wplist.cls_ships 						= menu.param[6]
	
	-- The trader who's waypoints we are going to list
	menu.wplist.trader 							= menu.param[7]
	
	-- The actual list of waypoints if passed back to this menu (from multi-wpadd or load)
	menu.wplist.waypoints						= menu.param[8] or  {}
	
	-- the waypoint returned by another screen (form add or edit)
	menu.wplist.waypoint 						= menu.param[9] or {}
	menu.wplist.waypoint_index					= menu.selrow
	
	-- Global options
	menu.wplist.GlobalOptions					= menu.param[10] or {}
	
	-- setup various variables used in the menu - titles
...
--	menu.wplist.waypointType = { "No Action", "Load", "Unload", "Buy", "Sell", "Fly to", "Refuel" }
	...
	menu.wplist.tracklog = {} 								-- DEPRACATED empty unless in reporting tracking menu
	...
	menu.wplist.CV = menu.wplist.trader[15]
	
	-- Deal with actions specified by the caller here
	if menu.wplist.action == "list" then
		-- Just resync the list ready for display
		for i, wp in ipairs(menu.wplist.waypoints) do
			wp[1] = i
		end
	elseif menu.wplist.action == "add" then
		-- add a new waypoint
	...
	elseif menu.wplist.action == "edit" then
...
Buttons.Back = function ()
	...
	menu.wplist.additional_params[1] = "nosave"
	Helper.closeMenuForSection(menu, false, "gMT_Logistics_ShipMenu", { {}, {}, menu.name, "", 
													menu.wplist.additional_params, {}, menu.wplist.trader, {}, {}, {} })
	menu.cleanup()
end
...
Buttons.Done = function()
	...
	table.insert( menu.wplist.trader, menu.wplist.CV )				-- true (1) if supplying CV

	-- return and save the updated trader
	menu.wplist.additional_params[1] = "save"
	Helper.closeMenuForSection(menu, false, "gMT_Logistics_ShipMenu", { {}, {}, menu.name, "", 
													menu.wplist.additional_params, {}, menu.wplist.trader, {}, {}, {} })
	menu.cleanup()
end
...
--	local new_waypoint = { menu.wplist.waypoint_index, 1, "None", "No Action", "None Selected", 0, 0, 0, 0, 0 } --  { index, valid, dest, wp_type, ware, min, max amount, price, range, activity override}
	...
	local self_row_state = { toprow, selrow, {} }
	local called_row_state = { 0, 0, {} }
	menu.wplist.additional_params = {"add", {0} }  -- {0} is placeholder for station ware list
	-- Select old or new style add list depending upon settings
	if 0 < menu.wplist.GlobalOptions[2] then 	-- pick the new one
		Helper.closeMenuForSection(menu, false, "gMT_Logistics_ListAdd", { called_row_state, self_row_state, menu.name, "", 
																				menu.wplist.additional_params, {}, menu.wplist.trader, 
																				{}, new_waypoint, {} })
	else 										-- pick the old one
		Helper.closeMenuForSection(menu, false, "gMT_Logistics_EditWayPoint", { called_row_state, self_row_state, menu.name, "", 
																				menu.wplist.additional_params, {}, menu.wplist.trader, 
																				{}, new_waypoint, {} })
	end
	menu.cleanup()
...
Buttons.EditWaypoint = function()
	...
	local self_row_state = { toprow, selrow, {} }
	local called_row_state = { 0, 0, {} }
	menu.wplist.additional_params = {"edit", {0} }  -- {0} is placeholder for station ware list
	Helper.closeMenuForSection(menu, false, "gMT_Logistics_EditWayPoint", { called_row_state, self_row_state, menu.name, "", 
																				menu.wplist.additional_params, {}, menu.wplist.trader, 
																				{}, selected_waypoint, {} })
	menu.cleanup()
