...
menu.onShowMenu = function ()
	...
	menu.ware 									= {}
	menu.wpedit 								= {} 
	menu.wpedit.toggle							= 0	-- for toggling error message
	
	-- Get the passed data
	-- Our opening row states
	menu.wpedit.toprow 							= menu.param[1][1] -- rows for return
	menu.wpedit.selrow 							= menu.param[1][2]

	-- Caller row states 
	menu.wpedit.caller_rowstate					= menu.param[2]

	-- name of menu that called us
	menu.wpedit.calling_menu 					= menu.param[3]
	
	-- Place to return if required
	menu.wpedit.return_menu						= menu.param[4]
	
	-- Additional parameters passed to this menu
	menu.wpedit.additional_params				= menu.param[5]
	menu.wpedit.action							= menu.wpedit.additional_params[1]
	menu.wpedit.warelist 						= menu.wpedit.additional_params[2] or {}
	menu.wpedit.newstation 						= menu.wpedit.additional_params[3] or nil


	-- Trade ship list (usually not passed)
	menu.wpedit.cls_ships 						= menu.param[6]
	
	-- The trader who's waypoints we are going to list
	menu.wpedit.trader 							= menu.param[7]
	
	-- The actual list of waypoints 
	menu.wpedit.waypoints						= menu.param[8] or  {}
	
	-- the waypoint passed or returned by another screen (form add or edit)
	menu.wpedit.waypoint 						= menu.param[9] or {}
	
	-- Global options
	menu.wpedit.GlobalOptions					= menu.param[10] or {}
...
--	menu.wpedit.waypointType = { 	"No Action", "Load", "Unload", "Buy", "Sell", "Fly to", "Refuel" }
	...
	if menu.wpedit.newstation then 
		...
		DebugError( GetComponentData(menu.wpedit.wp_station, "name") )
	end
	
	menu.wpedit.title = 		ReadText(150402,21)			-- menu.title = "MT Station Logistics - Administration"
	
	if menu.wpedit.action == "add" or menu.wpedit.action == "listswitch" or menu.wpedit.action == "mapswitch" then
...
menu.display = function ()	
	...
	local selectdesc = LibMT.create_body( menu, row_collection, cell_scripts, colWidths, false, false, headerHeight + 8, bodyHeight, menu.wpedit.toprow, menu.wpedit.selrow )
	...
	table.insert(menu.buttons, LibMT:BarButton())
	if menu.wpedit.wp_action == "addwaypoint" or menu.wpedit.wp_action == "listswitch" or menu.wpedit.wp_action == "mapswitch" then
		table.insert(menu.buttons, LibMT:BarButton( "List Select", Buttons.List, function () return true end, "INPUT_STATE_DETAILMONITOR_Y" ))
	else
		table.insert(menu.buttons, LibMT:BarButton())
	end
...
-- Callback function for waypoint editor back button    gMT_ShipMenu_LoadSaveReturn
Buttons.Back = function()
	local self_row_state = { 0, 0, {} }
	local additional_params = { "nosave" }
	Helper.closeMenuForSection(menu, false, "gMT_Logistics_WayPointList", { menu.wpedit.caller_rowstate, self_row_state, menu.name, "", 
																				additional_params, {}, menu.wpedit.trader, {}, {}, {} })
	menu.cleanup()
end

-- Callback function for waypoint editor switch to list button
Buttons.List = function()
--	local return_waypoint = menu.wpedit.waypoint
--	menu.wpedit.ret_arg[4] = "none"

	Helper.closeMenuForSection(menu, false, "gMT_WPListMenu_editwaypoint", { 0, 0, {}, {0,0,0,"listswitch"}, {}, menu.wpedit.trader, {} })
	menu.cleanup()
...
Buttons.Done = function()
	...
	local self_row_state = { 0, 0, {} }
	Helper.closeMenuForSection(menu, false, "gMT_Logistics_WayPointList", { menu.wpedit.caller_rowstate, self_row_state, menu.name, "", 
																				menu.wpedit.additional_params, {}, menu.wpedit.trader, 
																					menu.wpedit.waypoints, return_waypoint, menu.wpedit.GlobalOptions })
	menu.cleanup()
end
...
function Buttons:SelectHomebase()
	...
	-- Temporary save the working waypoint so we can add the homebase
	local new_wp_idx = menu.wpedit.waypoint[1]
	menu.wpedit.waypoint[1] = new_wp_idx
	menu.wpedit.waypoint[2] = menu.bool.IsWPActive
	menu.wpedit.waypoint[3] = menu.wpedit.trader[2]   -- homebase
	menu.wpedit.waypoint[4] = menu.wpedit.revwaypointType[menu.wpedit.selectedWPType]
	...
	local return_waypoint = menu.wpedit.waypoint
	local additional_params = { "selhomebase", 0, menu.wpedit.trader[2] }
	local self_row_state = { 0, 0 }
--	menu.wpedit.ret_arg[4] = menu.wpedit.wp_action
	Helper.closeMenuForSection(menu, false, "gMT_Logistics_EditWayPoint", { self_row_state, {}, menu.name, gMT_Logistics_EditWayPoint, 
																				additional_params, {}, menu.wpedit.trader, 
																					menu.wpedit.waypoints, return_waypoint, 
																						menu.wpedit.GlobalOptions} )
	menu.cleanup()
end
