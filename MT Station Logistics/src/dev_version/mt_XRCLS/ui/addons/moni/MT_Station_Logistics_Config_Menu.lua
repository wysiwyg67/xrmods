...
menu.onShowMenu = function ()
	...
	menu.config = {}
	menu.config.toprow 				= menu.param[1][1]
	menu.config.selrow 				= menu.param[1][2]
	
	menu.config.return_row_state	= menu.param[2]
	
	menu.config.additional_params 	= menu.param[5]
	menu.config.currentdbg 			= menu.config.additional_params[2]
	...
	if LibMT.DEBUG then
		menu.config.options = {
			{ReadText(150402,83), "gMT_ConfigMenu_debug", 0},
			{ReadText(150402,84), "gMT_ConfigMenu_debug", 1},
			{ReadText(150402,85), "gMT_ConfigMenu_debug", 2},
			{ReadText(150402,86), "gMT_ConfigMenu_debug", 3},
			{"Global Options", "gMT_ConfigMenu_global-options", 0},
...
menu.display = function ()	
	...
	local selectdesc = LibMT.create_body( menu, row_collection, cell_scripts, colWidths, true, false, headerHeight+6, bodyHeight - 45, menu.config.toprow, menu.config.selrow)
...
Buttons.Back = function()
	...
	local self_row_state = { toprow, selrow, {} }								-- Package up expand state
	local return_row_state = {1, 3, {}}											-- The state we want the return table to open in
...
Buttons.Select = function()
	...
	local selrow = Helper.currentDefaultTableRow
	local self_row_state = { toprow, selrow, {} }								-- Package up expand state
	-- options allow:- 	Change debug level 	- return to self
	--					Reset Data 			- return to self
	--					Uninstall			- return to game screen - trigger uninstall then cancel - through self?
	--					Global options		- Open Global Options Screen
	if menu.config.selection == "gMT_ConfigMenu_global-options" then -- Open Global Options Menu
		Helper.closeMenuForSection(menu, false, "gMT_Logistics_GlobalOptions", { {0,0,{}}, menu.config.return_row_state, menu.name, "", 
																					{}, {}, {}, {}, {}, {0} })
	else -- return to self 
		local additional_params = { menu.config.selection, menu.config.debug_num }
		Helper.closeMenuForSection(menu, false, "gMT_Logistics_ConfigMenu", { self_row_state, menu.config.return_row_state, menu.name, "", 
																				additional_params, {}, {}, {}, {}, {} })
	end
