...
LibMT.create_body = function( menu, row_collection, cell_scripts, colWidths, enable_border, doNotScale, headerHeight, bodyHeight, toprow, selrow )
	...
	local desc = setup.createCustomWidthTable(setup, colWidths, isColumnWidthsInPercent or false, doNotScale, 
											isBorderEnabled, tabOrder, fixedRows, offsetX, offsetY, bodyHeight, 
											false, toprow, selrow)
	return desc
end
...
--		DebugError( tostring(GetComponentData(ship, "name")) .. "  CargoBay Name: " .. tostring(cargobay.name) .. " Capacity:  " .. tostring(cargobay.capacity) )
		...
		-- Fix for Hermod that uses Hybrid cargo bay
		if string.find(cargobay.name, bulk) and string.find(cargobay.name, liquid) and not string.find(cargobay.name, energy) then
		...
		-- Fix for Lepton that uses Hybrid cargo bay
		if string.find(cargobay.name, container) and string.find(cargobay.name, energy)  and not string.find(cargobay.name, bulk)then
...
-- Function to sort 1-based list of stations into hashed zone sector cluster table
LibMT.SortStationsByZone = function( stations )
	local sorted_stations = {}

	-- iterate through station list 
	for _, station in ipairs( stations ) do
		local name, zone, sector, cluster = GetComponentData( station, "name", "zone", "sector", "cluster" )
		-- Check our base table for cluster sector and zone keys
		if sorted_stations[cluster] == nil then
			sorted_stations[cluster] = {}
		end
		if not sorted_stations[cluster][sector] then
			sorted_stations[cluster][sector] = {}
		end
		if not sorted_stations[cluster][sector][zone] then
			sorted_stations[cluster][sector][zone] = {}
		end
		
		table.insert( sorted_stations[cluster][sector][zone], {station, false} ) -- boolean is for selection 
	end
	
	return sorted_stations
end

LibMT.GetNumEntries = function( tab )
	local count = 0
...
-- Function to check that waypoint list is ok to load onto ship
LibMT.LoadWaypointsOK = function( ship, waypointlist )
	local retval = true -- assume true, assert false
	local warelist = {}

	-- Get all ware types in list
	for _, waypoint in ipairs(waypointlist) do
		local wp_type = waypoint[4]
		local ware = waypoint[5]
		if wp_type == 2 or wp_type == 3 or wp_type == 4 or wp_type == 5 or wp_type == 7 then  -- load, unload, buy, sell, refuel
			table.insert(warelist, ware)
		end
	end
	
	-- check each ware against ship 
	for _,ware in ipairs(warelist) do
		if not CheckSuitableTransportType( ship, ware) then
			retval = false
			break
		end
	end
...
-- function to convert waypoint packet into % amounts for storage in save list
LibMT.ConvertWayPointsToSave = function( ship, waypointlist)
	local returnlist = {}
	-- get ship cargo specs
	local cargo = ship[3]
	-- loop through list
	for i, waypoint in ipairs(waypointlist) do
		local maxamount = waypoint[7]
		local minamount = waypoint[6]
		local ware = waypoint[5]
		local wp_type = waypoint[4]
		if wp_type == 2 or wp_type == 3 or wp_type == 4 or wp_type == 5 or wp_type == 7 then  -- load, unload, buy, sell, refuel
			local ware_lib = GetLibraryEntry("wares", ware)
			local warevolume = ware_lib.volume
			local waretransport = GetWareData(ware, "transport")
			if cargo[6] > 0 and wp_type ~= 7 then  -- universal
				maxamount = Helper.round( ( (maxamount * warevolume * 100 )/ cargo[6]), 0)
				minamount = Helper.round( ( (minamount * warevolume * 100 )/ cargo[6]), 0)
			elseif CheckSuitableTransportType(ship[1], "nividium") and waretransport == "bulk" then
				maxamount = Helper.round( ( (maxamount * warevolume * 100 )/ cargo[2]), 0)
				minamount = Helper.round( ( (minamount * warevolume * 100 )/ cargo[2]), 0)
			elseif CheckSuitableTransportType(ship[1], "chemicalcompounds") and waretransport == "container" then
				maxamount = Helper.round( ( (maxamount * warevolume * 100 )/ cargo[3]), 0)
				minamount = Helper.round( ( (minamount * warevolume * 100 )/ cargo[3]), 0)
			elseif CheckSuitableTransportType(ship[1], "ioncells") and waretransport == "energy" then
				maxamount = Helper.round( ( (maxamount * warevolume * 100 )/ cargo[4]), 0)
				minamount = Helper.round( ( (minamount * warevolume * 100 )/ cargo[4]), 0)
			elseif CheckSuitableTransportType(ship[1], "ions") and waretransport == "liquid" then
				maxamount = Helper.round( ( (maxamount * warevolume * 100 )/ cargo[5]), 0)
				minamount = Helper.round( ( (minamount * warevolume * 100 )/ cargo[5]), 0)
			end
			if wp_type == 7 and CheckSuitableTransportType(ship[1], "fuelcells") then
				maxamount = 100 
				minamount = Helper.round( (minamount * 400/cargo[1]), 0) 
			end
			-- update table entries
			waypoint[7] = maxamount
			waypoint[6] = minamount
		end
		table.insert(returnlist, waypoint)
	end

	return returnlist
end

-- Function to convert saved waypoint list into ship list
LibMT.ConvertWayPointsToShip = function( ship, waypointlist )
	local returnlist = {}
	-- get ship cargo specs
	local cargo = ship[3]
	-- loop through list
	for i, waypoint in ipairs(waypointlist) do
		...
		if wp_type == 2 or wp_type == 3 or wp_type == 4 or wp_type == 5 or wp_type == 7 then  -- load, unload, buy, sell, refuel
			...
			local waretransport = GetWareData(ware, "transport")
			if cargo[6] > 0 and wp_type ~= 7 then  -- universal
				maxamount = math.floor( ( (cargo[6] * maxamount / 100) / warevolume  ), 0)
				minamount = math.floor( ( (cargo[6] * minamount) / (warevolume * 100 ) ), 0)
			elseif CheckSuitableTransportType(ship[1], "nividium") and waretransport == "bulk" then
				maxamount = math.floor( ( (cargo[2] * maxamount) / (warevolume * 100 ) ), 0)
				minamount = math.floor( ( (cargo[2] * minamount) / (warevolume * 100 ) ), 0)
			elseif CheckSuitableTransportType(ship[1], "chemicalcompounds") and waretransport == "container" then
				maxamount = math.floor( ( (cargo[3] * maxamount) / (warevolume * 100 ) ), 0)
				minamount = math.floor( ( (cargo[3] * minamount) / (warevolume * 100 ) ), 0)
			elseif CheckSuitableTransportType(ship[1], "ioncells") and waretransport == "energy" then
				maxamount = math.floor( ( (cargo[4] * maxamount) / (warevolume * 100 ) ), 0)
				minamount = math.floor( ( (cargo[4] * minamount) / (warevolume * 100 ) ), 0)
			elseif CheckSuitableTransportType(ship[1], "ions") and waretransport == "liquid" then
				maxamount = math.floor( ( (cargo[5] * maxamount) / (warevolume * 100 ) ), 0)
				minamount = math.floor( ( (cargo[5] * minamount) / (warevolume * 100 ) ), 0)
			end
			if wp_type == 7 and CheckSuitableTransportType(ship[1], "fuelcells") then
				maxamount = cargo[1] / 4 
				minamount = math.floor( (minamount * cargo[1] / 400), 0) 
			end
			-- update table entries
			waypoint[7] = maxamount
			waypoint[6] = minamount
